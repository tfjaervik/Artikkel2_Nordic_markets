...
delisting_date <- read_excel("data/worldscope_datastream/delisted/copenhagen_delisted_ISIN_delisting_date.xlsx",
...
dk_delisted$Name %>% duplicated() %>% which()

# Check duplicated mnemonics

dk_delisted$Symbol %>% duplicated() %>% which()
...
str_starts(dk_delisted$Name, "WC") %>% sum()

# No. This means that we can remove all columns that start with "WC", since these 
# do not contain any data

# remove_wc_col <- which(str_starts(colnames(dk_delisted), "WC"))
remove_wc_col <- str_starts(colnames(dk_delisted), "WC") # Rewrote so that we do not remove all dataframe when vector is empty

dk_delisted <- dk_delisted[, !remove_wc_col]
...
missing_data <- str_detect(dk_delisted[2, ] %>% unlist(), "ER")
dk_delisted <- dk_delisted[, !missing_data]
rm(missing_data)


# Let's also remove any stocks that do not have six mnemonics in a row. 

mnemonics_vec <- dk_delisted[1, 4:ncol(dk_delisted)] %>% unlist() %>% unname()
...
dk_delisted <- dk_delisted[, -delete_columns]
rm(delete_columns)
...
# Also remove the name and symbol column from dk_delisted and the rows exceeding
# the number of dates



df_mnemonics <- dk_delisted[, 1:2]
...
dk_delisted <- dk_delisted[, -c(1,2)] 
dk_delisted <- dk_delisted[1:316, ]
...
for(i in 1:length(mnemonics)){
  ...
  replacement_string <- colnames(dk_delisted[test_1 + 1])
  ...
  colnames(dk_delisted)[test_1 + 1] <- replacement_string
...
test_string <- colnames(dk_delisted)[2]
...
colnames_aux <- vector(mode = "character", length = length(colnames(dk_delisted))-1)

for(i in 2:length(colnames(dk_delisted))){
  
  if(str_detect(colnames(dk_delisted)[i], " - OPERATING INCOME")){
    colnames_aux[i-1] <- paste0(sub(" - OPERATING INCOME",  paste0( " ", dk_delisted[1, i]), x = colnames(dk_delisted)[i]), " - OPERATING INCOME")
  } else if((str_detect(colnames(dk_delisted)[i], " - TOTAL ASSETS"))){
    colnames_aux[i-1] <- paste0(sub(" - TOTAL ASSETS", paste0( " ", dk_delisted[1, i]), x = colnames(dk_delisted)[i]), " - TOTAL ASSETS")
  } else if((str_detect(colnames(dk_delisted)[i], " - TOTAL LIABILITIES"))){
    colnames_aux[i-1] <- paste0(sub(" - TOTAL LIABILITIES", paste0( " ", dk_delisted[1, i]), x = colnames(dk_delisted)[i]), " - TOTAL LIABILITIES")
  } else if((str_detect(colnames(dk_delisted)[i], " - COMMON SHARES OUTSTANDING"))){
    colnames_aux[i-1] <- paste0(sub(" - COMMON SHARES OUTSTANDING", paste0( " ", dk_delisted[1, i]), x = colnames(dk_delisted)[i]), " - COMMON SHARES OUTSTANDING")
  } else if((str_detect(colnames(dk_delisted)[i], " - PRICE"))){
    colnames_aux[i-1] <- paste0(sub(" - PRICE", paste0( " ", dk_delisted[1, i]), x = colnames(dk_delisted)[i]), " - PRICE")
  } else if((str_detect(colnames(dk_delisted)[i], " - TOT RETURN IND"))){
    colnames_aux[i-1] <- paste0(sub(" - TOT RETURN IND", paste0( " ", dk_delisted[1, i]), x = colnames(dk_delisted)[i]), " - TOT RETURN IND")
  }
  
}

colnames(dk_delisted) <- c("date", colnames_aux)
# Check for duplicated column names

duplicated_columns_last <- which(duplicated(colnames(dk_delisted)))
duplicated_columns_first <- which(duplicated(colnames(dk_delisted), fromLast = TRUE))

# Now we have no duplicated column names

# duplicated_first_df <- dk_delisted[, duplicated_columns_first]
# duplicated_last_df <- dk_delisted[, duplicated_columns_last]
# 
...
# 
# delete_columns <- which(dk_delisted[1, ] %in% delete_mnemonics)
# 
# dk_delisted <- dk_delisted[, -delete_columns]
# 
# dk_delisted[, duplicated_columns] %>% View()
...
dk_delisted_long <- dk_delisted_long %>% 
...
save(dk_delisted_long, file = "data/rdata/cleaned_data_for_analysis/ff-factors/denmark_delisted_cleaned.Rdata")
