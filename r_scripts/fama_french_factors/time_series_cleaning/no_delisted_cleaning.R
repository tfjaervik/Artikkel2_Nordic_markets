...
no_delisted <- read_excel("data/worldscope_datastream/delisted/worldscope_oslo_delisted_returns_mnemonic.xlsx")

delisting_date <- read_excel("data/worldscope_datastream/delisted/oslo_delisted_ISIN_delisting_date.xlsx",
...
no_delisted$Name %>% duplicated() %>% which()

# Check duplicated mnemonics

no_delisted$Symbol %>% duplicated() %>% which()
...
colnames(no_delisted)[3] <- "date"

# Does any company names start with WC?

str_starts(no_delisted$Name, "WC") %>% sum()

# No. This means that we can remove all columns that start with "WC", since these 
# do not contain any data

remove_wc_col <- which(str_starts(colnames(no_delisted), "WC"))

no_delisted <- no_delisted[, -remove_wc_col]

rm(remove_wc_col)

# Make all columns numeric except "date".

# no_delisted <- no_delisted %>%
...
colnames(no_delisted) <- str_remove_all(colnames(no_delisted), "\\.\\.\\.(.+)")



# Remove all columns that are named "#ERROR"

remove_columns_1 <- which(stringr::str_detect(colnames(no_delisted), "#ERROR"))
no_delisted <- no_delisted[, -remove_columns_1]
rm(remove_columns_1)


# Remove columns without names

# empty_columns <- colnames(no_delisted) %>% 
#   is.na() %>%
#   which()

# no_delisted <- no_delisted[, -empty_columns]
# rm(empty_columns)

# Identify which columns are price columns (except the first three columns)

# price_columns <- which(!stringr::str_detect(colnames(no_delisted), " - "))

price_columns <- which(!(str_ends(colnames(no_delisted), " - OPERATING INCOME") | 
                           str_ends(colnames(no_delisted), " - TOTAL ASSETS") |
                           str_ends(colnames(no_delisted), " - TOTAL LIABILITIES") |
                           str_ends(colnames(no_delisted), " - COMMON SHARES OUTSTANDING") |
                           str_ends(colnames(no_delisted), " - TOT RETURN IND")))
...
for(i in price_columns){
  # if(!(str_ends(colnames(no_delisted[i]), " - OPERATING INCOME") | 
  #      str_ends(colnames(no_delisted[i]), " - TOTAL ASSETS") |
  #      str_ends(colnames(no_delisted[i]), " - TOTAL LIABILITIES") |
  #      str_ends(colnames(no_delisted[i]), " - COMMON SHARES OUTSTANDING") |
  #      str_ends(colnames(no_delisted[i]), " - TOT RETURN IND"))){
  #   
  #   colnames(no_delisted)[i] <- paste0(colnames(no_delisted[i]), " - PRICE")
  #   
  # }
  colnames(no_delisted)[i] <- paste0(colnames(no_delisted[i]), " - PRICE")
}
...
missing_data <- which(str_detect(no_delisted[2, ] %>% unlist(), "ER"))
no_delisted <- no_delisted[, -missing_data]
rm(missing_data)


# Let's also remove any stocks that do not have six mnemonics in a row. 

mnemonics_vec <- no_delisted[1, 4:ncol(no_delisted)] %>% unlist() %>% unname()
...
mnemonics_match <- no_delisted[1, 4:ncol(no_delisted)] %>% unlist()
...
no_delisted <- no_delisted[, -delete_columns]
rm(delete_columns)
...
# Also remove the name and symbol column from no_delisted and the rows exceeding
# the number of dates



df_mnemonics <- no_delisted[, 1:2]


mnemonics <- no_delisted[, 2] %>% drop_na() %>% unlist()
mnemonics_match <- no_delisted[1, 4:ncol(no_delisted)] %>% unlist()
keep_mnemonics <- which(mnemonics %in% mnemonics_match)
mnemonics <- mnemonics[keep_mnemonics]

no_delisted <- no_delisted[, -c(1,2)] 
no_delisted <- no_delisted[1:316, ]
...
for(i in 1:length(mnemonics)){
  ...
  replacement_string <- colnames(no_delisted[test_1 + 1])
  ...
  colnames(no_delisted)[test_1 + 1] <- replacement_string
  
  
  # colnames(no_delisted)[test_1 + 3] <- str_replace(colnames(no_delisted)[test_1 + 3],  
  #                                                str_sub(colnames(no_delisted)[test_1 + 3], 
  #                                                        1, 
  #                                                        str_locate(colnames(no_delisted)[test_1 + 3], " -")[1]-1),
  #                                                mnemonics_list[[mnemonics[i]]])
...
test_string <- colnames(no_delisted)[2]
...
colnames_aux <- vector(mode = "character", length = length(colnames(no_delisted))-1)

for(i in 2:length(colnames(no_delisted))){
  
  if(str_detect(colnames(no_delisted)[i], " - OPERATING INCOME")){
    colnames_aux[i-1] <- paste0(sub(" - OPERATING INCOME",  paste0( " ", no_delisted[1, i]), x = colnames(no_delisted)[i]), " - OPERATING INCOME")
  } else if((str_detect(colnames(no_delisted)[i], " - TOTAL ASSETS"))){
    colnames_aux[i-1] <- paste0(sub(" - TOTAL ASSETS", paste0( " ", no_delisted[1, i]), x = colnames(no_delisted)[i]), " - TOTAL ASSETS")
  } else if((str_detect(colnames(no_delisted)[i], " - TOTAL LIABILITIES"))){
    colnames_aux[i-1] <- paste0(sub(" - TOTAL LIABILITIES", paste0( " ", no_delisted[1, i]), x = colnames(no_delisted)[i]), " - TOTAL LIABILITIES")
  } else if((str_detect(colnames(no_delisted)[i], " - COMMON SHARES OUTSTANDING"))){
    colnames_aux[i-1] <- paste0(sub(" - COMMON SHARES OUTSTANDING", paste0( " ", no_delisted[1, i]), x = colnames(no_delisted)[i]), " - COMMON SHARES OUTSTANDING")
  } else if((str_detect(colnames(no_delisted)[i], " - PRICE"))){
    colnames_aux[i-1] <- paste0(sub(" - PRICE", paste0( " ", no_delisted[1, i]), x = colnames(no_delisted)[i]), " - PRICE")
  } else if((str_detect(colnames(no_delisted)[i], " - TOT RETURN IND"))){
    colnames_aux[i-1] <- paste0(sub(" - TOT RETURN IND", paste0( " ", no_delisted[1, i]), x = colnames(no_delisted)[i]), " - TOT RETURN IND")
  }
  
}

colnames(no_delisted) <- c("date", colnames_aux)
# Check for duplicated column names

duplicated_columns_last <- which(duplicated(colnames(no_delisted)))
duplicated_columns_first <- which(duplicated(colnames(no_delisted), fromLast = TRUE))

# Now we have no duplicated column names

# duplicated_first_df <- no_delisted[, duplicated_columns_first]
# duplicated_last_df <- no_delisted[, duplicated_columns_last]
# 
...
# 
# delete_columns <- which(no_delisted[1, ] %in% delete_mnemonics)
# 
# no_delisted <- no_delisted[, -delete_columns]
# 
# no_delisted[, duplicated_columns] %>% View()
# Pivot data to long format

# Can now remove the first row containing only mnemonics and pivot the data

no_delisted <- no_delisted[-1, ]
...
no_delisted_long <- no_delisted_long %>% 
...
save(no_delisted_long, file = "data/rdata/cleaned_data_for_analysis/ff-factors/norway_delisted_cleaned.Rdata")
