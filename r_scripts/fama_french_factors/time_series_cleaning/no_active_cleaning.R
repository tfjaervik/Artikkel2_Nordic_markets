...
no_active <- read_excel("data/worldscope_datastream/active/worldscope_oslo_active_returns_mnemonic.xlsx",
                        col_types = c("guess", "guess", "date", rep("guess", times = 1173-3)))
...
colnames(no_active)[1] <- "Name"

no_active$Name %>% duplicated() %>% which() 

# Check duplicated mnemonics

no_active$Symbol %>% duplicated() %>% which()
...
colnames(no_active)[3] <- "date"

# Does any company names start with WC?

str_starts(no_active$Name, "WC") %>% sum(na.rm = TRUE)

# No. This means that we can remove all columns that start with "WC", since these 
# do not contain any data

# remove_wc_col <- which(str_starts(colnames(no_active), "WC"))
remove_wc_col <- str_starts(colnames(no_active), "WC") # Rewrote so that we do not remove all dataframe when vector is empty

no_active <- no_active[, !remove_wc_col]
...
colnames(no_active) <- str_remove_all(colnames(no_active), "\\.\\.\\.(.+)")



# Remove all columns that are named "#ERROR"

remove_columns_1 <- which(stringr::str_detect(colnames(no_active), "#ERROR"))
no_active <- no_active[, -remove_columns_1]
rm(remove_columns_1)


# Remove columns without names

# empty_columns <- colnames(no_active) %>% 
#   is.na() %>%
#   which()

# no_active <- no_active[, -empty_columns]
# rm(empty_columns)

# Identify which columns are price columns (except the first three columns)

# price_columns <- which(!stringr::str_detect(colnames(no_active), " - "))

price_columns <- which(!(str_ends(colnames(no_active), " - OPERATING INCOME") | 
                           str_ends(colnames(no_active), " - TOTAL ASSETS") |
                           str_ends(colnames(no_active), " - TOTAL LIABILITIES") |
                           str_ends(colnames(no_active), " - COMMON SHARES OUTSTANDING") |
                           str_ends(colnames(no_active), " - TOT RETURN IND")))
...
for(i in price_columns){
  # if(!(str_ends(colnames(no_active[i]), " - OPERATING INCOME") | 
  #      str_ends(colnames(no_active[i]), " - TOTAL ASSETS") |
  #      str_ends(colnames(no_active[i]), " - TOTAL LIABILITIES") |
  #      str_ends(colnames(no_active[i]), " - COMMON SHARES OUTSTANDING") |
  #      str_ends(colnames(no_active[i]), " - TOT RETURN IND"))){
  #   
  #   colnames(no_active)[i] <- paste0(colnames(no_active[i]), " - PRICE")
  #   
  # }
  colnames(no_active)[i] <- paste0(colnames(no_active[i]), " - PRICE")
}
...
missing_data <- str_detect(no_active[2, ] %>% unlist(), "ER")
no_active <- no_active[, !missing_data]
rm(missing_data)


# Let's also remove any stocks that do not have six mnemonics in a row. 

mnemonics_vec <- no_active[1, 4:ncol(no_active)] %>% unlist() %>% unname()
...
mnemonics_match <- no_active[1, 4:ncol(no_active)] %>% unlist()
...
no_active <- no_active[, -delete_columns]
rm(delete_columns)
...
# Also remove the name and symbol column from no_active and the rows exceeding
# the number of dates



df_mnemonics <- no_active[, 1:2]


mnemonics <- no_active[, 2] %>% drop_na() %>% unlist()
mnemonics_match <- no_active[1, 4:ncol(no_active)] %>% unlist()
keep_mnemonics <- which(mnemonics %in% mnemonics_match)
mnemonics <- mnemonics[keep_mnemonics]

no_active <- no_active[, -c(1,2)] 
no_active <- no_active[1:316, ]
...
no_active_copy <- no_active

# Must change colnames of 2 companies beforehand because it generates an error

# colnames(no_active)[620:625] <- c("HEMNET GROUP AB PUB - OPERATING INCOME",
...
# 
# colnames(no_active)[1946:1951] <- c("WESC AB - OPERATING INCOME",
...
colnames(no_active) <- gsub("\\s*\\([^\\)]+\\)","",as.character(colnames(no_active))) 
...
for(i in 1:length(mnemonics)){
  ...
  replacement_string <- colnames(no_active[test_1 + 1])
  ...
  colnames(no_active)[test_1 + 1] <- replacement_string
  
  
  # colnames(no_active)[test_1 + 3] <- str_replace(colnames(no_active)[test_1 + 3],  
  #                                                str_sub(colnames(no_active)[test_1 + 3], 
  #                                                        1, 
  #                                                        str_locate(colnames(no_active)[test_1 + 3], " -")[1]-1),
  #                                                mnemonics_list[[mnemonics[i]]])
...
test_string <- colnames(no_active)[2]
...
colnames_aux <- vector(mode = "character", length = length(colnames(no_active))-1)

for(i in 2:length(colnames(no_active))){
  
  if(str_detect(colnames(no_active)[i], " - OPERATING INCOME")){
    colnames_aux[i-1] <- paste0(sub(" - OPERATING INCOME",  paste0( " ", no_active[1, i]), x = colnames(no_active)[i]), " - OPERATING INCOME")
  } else if((str_detect(colnames(no_active)[i], " - TOTAL ASSETS"))){
    colnames_aux[i-1] <- paste0(sub(" - TOTAL ASSETS", paste0( " ", no_active[1, i]), x = colnames(no_active)[i]), " - TOTAL ASSETS")
  } else if((str_detect(colnames(no_active)[i], " - TOTAL LIABILITIES"))){
    colnames_aux[i-1] <- paste0(sub(" - TOTAL LIABILITIES", paste0( " ", no_active[1, i]), x = colnames(no_active)[i]), " - TOTAL LIABILITIES")
  } else if((str_detect(colnames(no_active)[i], " - COMMON SHARES OUTSTANDING"))){
    colnames_aux[i-1] <- paste0(sub(" - COMMON SHARES OUTSTANDING", paste0( " ", no_active[1, i]), x = colnames(no_active)[i]), " - COMMON SHARES OUTSTANDING")
  } else if((str_detect(colnames(no_active)[i], " - PRICE"))){
    colnames_aux[i-1] <- paste0(sub(" - PRICE", paste0( " ", no_active[1, i]), x = colnames(no_active)[i]), " - PRICE")
  } else if((str_detect(colnames(no_active)[i], " - TOT RETURN IND"))){
    colnames_aux[i-1] <- paste0(sub(" - TOT RETURN IND", paste0( " ", no_active[1, i]), x = colnames(no_active)[i]), " - TOT RETURN IND")
  }
  
}

colnames(no_active) <- c("date", colnames_aux)
# Check for duplicated column names

duplicated_columns_last <- which(duplicated(colnames(no_active)))
duplicated_columns_first <- which(duplicated(colnames(no_active), fromLast = TRUE))

# Now we have no duplicated column names

# duplicated_first_df <- no_active[, duplicated_columns_first]
# duplicated_last_df <- no_active[, duplicated_columns_last]
# 
...
# 
# delete_columns <- which(no_active[1, ] %in% delete_mnemonics)
# 
# no_active <- no_active[, -delete_columns]
# 
# no_active[, duplicated_columns] %>% View()
...
save(no_active_long, file = "data/rdata/cleaned_data_for_analysis/ff-factors/norway_active_cleaned.Rdata")
