...
se_active <- read_excel("data/worldscope_datastream/active/worldscope_stockholm_active_returns_mnemonic.xlsx",
                        col_types = c("guess", "guess", "date", rep("guess", times = 2367-3)))
...
colnames(se_active)[1] <- "Name"

se_active$Name %>% duplicated() %>% which() 

# Check duplicated mnemonics

se_active$Symbol %>% duplicated() %>% which()
...
colnames(se_active)[3] <- "date"

# Does any company names start with WC?

str_starts(se_active$Name, "WC") %>% sum(na.rm = TRUE)

# No. This means that we can remove all columns that start with "WC", since these 
# do not contain any data

# remove_wc_col <- which(str_starts(colnames(se_active), "WC"))
remove_wc_col <- str_starts(colnames(se_active), "WC") # Rewrote so that we do not remove all dataframe when vector is empty

se_active <- se_active[, !remove_wc_col]
...
colnames(se_active) <- str_remove_all(colnames(se_active), "\\.\\.\\.(.+)")



# Remove all columns that are named "#ERROR"

remove_columns_1 <- which(stringr::str_detect(colnames(se_active), "#ERROR"))
se_active <- se_active[, -remove_columns_1]
rm(remove_columns_1)


# Remove columns without names

# empty_columns <- colnames(se_active) %>% 
#   is.na() %>%
#   which()

# se_active <- se_active[, -empty_columns]
# rm(empty_columns)

# Identify which columns are price columns (except the first three columns)

# price_columns <- which(!stringr::str_detect(colnames(se_active), " - "))

price_columns <- which(!(str_ends(colnames(se_active), " - OPERATING INCOME") | 
                           str_ends(colnames(se_active), " - TOTAL ASSETS") |
                           str_ends(colnames(se_active), " - TOTAL LIABILITIES") |
                           str_ends(colnames(se_active), " - COMMON SHARES OUTSTANDING") |
                           str_ends(colnames(se_active), " - TOT RETURN IND")))
...
for(i in price_columns){
  # if(!(str_ends(colnames(se_active[i]), " - OPERATING INCOME") | 
  #      str_ends(colnames(se_active[i]), " - TOTAL ASSETS") |
  #      str_ends(colnames(se_active[i]), " - TOTAL LIABILITIES") |
  #      str_ends(colnames(se_active[i]), " - COMMON SHARES OUTSTANDING") |
  #      str_ends(colnames(se_active[i]), " - TOT RETURN IND"))){
  #   
  #   colnames(se_active)[i] <- paste0(colnames(se_active[i]), " - PRICE")
  #   
  # }
  colnames(se_active)[i] <- paste0(colnames(se_active[i]), " - PRICE")
}
...
missing_data <- str_detect(se_active[2, ] %>% unlist(), "ER")
se_active <- se_active[, !missing_data]
rm(missing_data)


# Let's also remove any stocks that do not have six mnemonics in a row. 

mnemonics_vec <- se_active[1, 4:ncol(se_active)] %>% unlist() %>% unname()
...
mnemonics_match <- se_active[1, 4:ncol(se_active)] %>% unlist()
...
se_active <- se_active[, -delete_columns]
rm(delete_columns)
...
# Also remove the name and symbol column from se_active and the rows exceeding
# the number of dates



df_mnemonics <- se_active[, 1:2]


mnemonics <- se_active[, 2] %>% drop_na() %>% unlist()
mnemonics_match <- se_active[1, 4:ncol(se_active)] %>% unlist()
keep_mnemonics <- which(mnemonics %in% mnemonics_match)
mnemonics <- mnemonics[keep_mnemonics]

se_active <- se_active[, -c(1,2)] 
se_active <- se_active[1:316, ]
...
se_active_copy <- se_active

# Must change colnames of 2 companies beforehand because it generates an error

# colnames(se_active)[620:625] <- c("HEMNET GROUP AB PUB - OPERATING INCOME",
...
# 
# colnames(se_active)[1946:1951] <- c("WESC AB - OPERATING INCOME",
...
colnames(se_active) <- gsub("\\s*\\([^\\)]+\\)","",as.character(colnames(se_active))) 
...
for(i in 1:length(mnemonics)){
  ...
  replacement_string <- colnames(se_active[test_1 + 1])
  ...
  colnames(se_active)[test_1 + 1] <- replacement_string
  
  
  # colnames(se_active)[test_1 + 3] <- str_replace(colnames(se_active)[test_1 + 3],  
  #                                                str_sub(colnames(se_active)[test_1 + 3], 
  #                                                        1, 
  #                                                        str_locate(colnames(se_active)[test_1 + 3], " -")[1]-1),
  #                                                mnemonics_list[[mnemonics[i]]])
...
test_string <- colnames(se_active)[2]
...
colnames_aux <- vector(mode = "character", length = length(colnames(se_active))-1)

for(i in 2:length(colnames(se_active))){
  
  if(str_detect(colnames(se_active)[i], " - OPERATING INCOME")){
    colnames_aux[i-1] <- paste0(sub(" - OPERATING INCOME",  paste0( " ", se_active[1, i]), x = colnames(se_active)[i]), " - OPERATING INCOME")
  } else if((str_detect(colnames(se_active)[i], " - TOTAL ASSETS"))){
    colnames_aux[i-1] <- paste0(sub(" - TOTAL ASSETS", paste0( " ", se_active[1, i]), x = colnames(se_active)[i]), " - TOTAL ASSETS")
  } else if((str_detect(colnames(se_active)[i], " - TOTAL LIABILITIES"))){
    colnames_aux[i-1] <- paste0(sub(" - TOTAL LIABILITIES", paste0( " ", se_active[1, i]), x = colnames(se_active)[i]), " - TOTAL LIABILITIES")
  } else if((str_detect(colnames(se_active)[i], " - COMMON SHARES OUTSTANDING"))){
    colnames_aux[i-1] <- paste0(sub(" - COMMON SHARES OUTSTANDING", paste0( " ", se_active[1, i]), x = colnames(se_active)[i]), " - COMMON SHARES OUTSTANDING")
  } else if((str_detect(colnames(se_active)[i], " - PRICE"))){
    colnames_aux[i-1] <- paste0(sub(" - PRICE", paste0( " ", se_active[1, i]), x = colnames(se_active)[i]), " - PRICE")
  } else if((str_detect(colnames(se_active)[i], " - TOT RETURN IND"))){
    colnames_aux[i-1] <- paste0(sub(" - TOT RETURN IND", paste0( " ", se_active[1, i]), x = colnames(se_active)[i]), " - TOT RETURN IND")
  }
  
}

colnames(se_active) <- c("date", colnames_aux)
# Check for duplicated column names

duplicated_columns_last <- which(duplicated(colnames(se_active)))
duplicated_columns_first <- which(duplicated(colnames(se_active), fromLast = TRUE))

# Now we have no duplicated column names

# duplicated_first_df <- se_active[, duplicated_columns_first]
# duplicated_last_df <- se_active[, duplicated_columns_last]
# 
...
# 
# delete_columns <- which(se_active[1, ] %in% delete_mnemonics)
# 
# se_active <- se_active[, -delete_columns]
# 
# se_active[, duplicated_columns] %>% View()
# Pivot data to long format

# Can now remove the first row containing only mnemonics and pivot the data

se_active <- se_active[-1, ]
...
save(se_active_long, file = "data/rdata/cleaned_data_for_analysis/ff-factors/sweden_active_cleaned.Rdata")
