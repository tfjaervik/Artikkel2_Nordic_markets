...
df <- df %>% 
  ...
  mutate(beta_quantile = rank(beta, ties.method = "first")/length(beta),
         beta_quintile = case_when(beta_quantile <= 0.2 ~ 1,
                                   beta_quantile > 0.2 & beta_quantile <= 0.4 ~ 2,
                                   beta_quantile > 0.4 & beta_quantile <= 0.6 ~ 3,
                                   beta_quantile > 0.6 & beta_quantile <= 0.8 ~ 4,
                                   beta_quantile > 0.8 & beta_quantile <= 1 ~ 5)) %>% 
  group_by(YearMonth, beta_quintile) %>% 
...
fut_monthly_ret <- df %>% 
  group_by(YearMonth, beta_quintile, ltd_quintile) %>% 
...
# beta_quantile and date in each city, i.e. 1 per quintile for each date in each city. This is 
# because we only need the future return for each ltd_quintile, beta_quintile, YearMonth and city.
...
# Check that there are 5 quintiles for every YearMonth for every city and beta_quintile 
...
fut_monthly_ret <- fut_monthly_ret %>% 
  select(YearMonth, beta_quintile, ltd_quintile, future_ret) %>% 
  arrange(YearMonth, beta_quintile, ltd_quintile) %>% 
  ungroup()


# Remember that we have to calculate it by grouping on city. 
...
calculate_double_sorts <- function(x, col_name){
  ...
  aux_beta <- as.numeric(substr(col_name, 1, 1))
  
  aux_df <- fut_monthly_ret_2 %>% 
    filter(beta_quintile == aux_beta)
...
calculate_double_sorts(fut_monthly_ret, col_name = "1 Low \beta")


named_sorts <- c("1 Low beta", "2", "3", "4", "5 High beta")
...
table_3 <- kable(estimates_df,
                 ...
                 col.names = c("Quintile", "1 Low $\\beta$", 
                               "2", "3", "4", "5 High $\\beta$"),
                 caption = "Future returns sorted on beta and LTD",
                 label = "beta_ltd_sorted_future_returns_equal_weighted") %>% 
  kable_styling() %>% 
...
save(table_3, file = "data/rdata/rdata_tables/double_sorts_beta_ltd_equal_weighted.Rdata")

# Load the table and set the positioning

load("data/rdata/rdata_tables/double_sorts_beta_ltd_equal_weighted.Rdata")
table_3 <- table_3 %>% 
  TexTools::ltx_placement(tbl_placement = "H")


TexTools::write_tex(table_3, path = "_tables/double_sorts_beta_ltd_equal_weighted") 
