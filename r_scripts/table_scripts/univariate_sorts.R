...
# load("data/rdata/rdata_multinational/stock_data_monthly.Rdata")
...
df <- df %>% 
  ...
  ungroup() %>%
  group_by(YearMonth) %>% 
  mutate(quantile = rank(utd, ties.method = "first")/length(utd),
         quintile = case_when(quantile <= 0.2 ~ 1,
                              quantile > 0.2 & quantile <= 0.4 ~ 2,
                              quantile > 0.4 & quantile <= 0.6 ~ 3,
                              quantile > 0.6 & quantile <= 0.8 ~ 4,
  ...
  # pull(n) %>% 
  # unique()
  #        #quintile_2 = quantile(utd, seq(0, 1, 0.2)),
  #        #quintile_3 = assign_quintile(utd)) %>% 
...
sd_diff_ret_newey_west_utd <- lm(fut_monthly_ret_2$diff_ret ~ 1) %>% 
  lmtest::coeftest(., vcov = 
                     sandwich::NeweyWest(
                       lm(fut_monthly_ret_2$diff_ret ~ 1))
  ) %>% 
...
sd_diff_ret_utd <- lm(fut_monthly_ret_2$diff_ret ~ 1) %>% 
  broom::tidy() %>% 
  .$std.error
...
univariate_results <- fut_monthly_ret %>%
  ...
  mutate(Return = round(Return*100, digits = 3),
         Return = add_significance_level(data = Return, p_value = p_value),
         p_value = round(p_value, digits = 3),
         Quintile = case_when(Quintile == "1" ~ "1 Weak UTD",
                              Quintile == "5" ~ "5 Strong UTD",
                              TRUE ~ Quintile))
...
univariate_sorts_table <- kable(univariate_results,
                                ...
                                booktabs = T,
                                col.names = c("Quintile", "Returns", "P-value"),
                                caption = "Future returns sorted on UTD",
                                label = "UTD_sorted_future_returns") %>% 
  kable_styling() %>% 
  TexTools::ltx_caption(., tbl_note = 
                          "Stocks have been sorted into quintiles based on their
...
TexTools::write_tex(univariate_sorts_table, path = "_tables/univariate_sorts_UTD")
