...
fut_monthly_ret <- df %>% 
  group_by(YearMonth, coskewness_quintile, ltd_quintile) %>% 
...
# because we only need the future return for each ltd_quintile, coskewness_quintile, YearMonth and city.
...
fut_monthly_ret <- fut_monthly_ret %>% 
  select(YearMonth, coskewness_quintile, ltd_quintile, future_ret) %>% 
  arrange(YearMonth, coskewness_quintile, ltd_quintile) %>% 
  ungroup()
...
table_3 <- kable(estimates_df,
                 ...
                 label = "coskewness_ltd_sorted_future_returns_equal_weighted") %>% 
  kable_styling() %>% 
...
save(table_3, file = "data/rdata/rdata_tables/double_sorts_coskewness_ltd_equal_weighted.Rdata")

# Load the table and set the positioning

load("data/rdata/rdata_tables/double_sorts_coskewness_ltd_equal_weighted.Rdata")
table_3 <- table_3 %>% 
  TexTools::ltx_placement(tbl_placement = "H")


TexTools::write_tex(table_3, path = "_tables/double_sorts_coskewness_ltd_equal_weighted") 
