...
df <- df %>% 
  ...
  group_by(YearMonth, beta_quintile, ltd_nll_quintile) %>% 
...
# because we only need the future return for each ltd_nll_quintile, beta_quintile, YearMonth and city.
...
fut_monthly_ret <- fut_monthly_ret %>% 
  select(YearMonth, beta_quintile, ltd_nll_quintile, future_ret) %>% 
  arrange(YearMonth, beta_quintile, ltd_nll_quintile) %>% 
  ungroup()
...
table_3 <- kable(estimates_df,
                 ...
                 label = "beta_ltd_nll_sorted_future_returns") %>% 
  kable_styling() %>% 
...
save(table_3, file = "data/rdata/rdata_tables/double_sorts_beta_ltd_nll.Rdata")

# Load the table and set the positioning

load("data/rdata/rdata_tables/double_sorts_beta_ltd_nll.Rdata")
table_3 <- table_3 %>% 
  TexTools::ltx_placement(tbl_placement = "H")


TexTools::write_tex(table_3, path = "_tables/double_sorts_beta_ltd_nll")    
