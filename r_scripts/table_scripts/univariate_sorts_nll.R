...
df <- df %>% 
  ...
  ungroup() %>%
  group_by(YearMonth) %>% 
  mutate(quantile = rank(ltd_nll, ties.method = "first")/length(ltd_nll),
  ...
  # pull(n) %>% 
  # unique()
  #        #quintile_2 = quantile(ltd_nll, seq(0, 1, 0.2)),
  #        #quintile_3 = assign_quintile(ltd_nll)) %>% 
...
univariate_sorts_table <- kable(univariate_results,
                                ...
                                label = "LTD_nll_sorted_future_returns") %>% 
  kable_styling() %>% 
...
TexTools::write_tex(univariate_sorts_table, path = "_tables/univariate_sorts_nll")
