...
fut_monthly_ret <- df %>% 
  group_by(YearMonth, downside_beta_quintile, ltd_quintile) %>% 
...
# because we only need the future return for each ltd_quintile, downside_beta_quintile, YearMonth and city.
...
fut_monthly_ret <- fut_monthly_ret %>% 
  select(YearMonth, downside_beta_quintile, ltd_quintile, future_ret) %>% 
  arrange(YearMonth, downside_beta_quintile, ltd_quintile) %>% 
  ungroup()
...
calculate_double_sorts(fut_monthly_ret, col_name = "1 Low downside beta")


named_sorts <- c("1 Low beta", "2", "3", "4", "5 High downside beta")
...
table_3 <- kable(estimates_df,
                 ...
                 label = "downside_beta_ltd_sorted_future_returns_equal_weighted") %>% 
  kable_styling() %>% 
...
save(table_3, file = "data/rdata/rdata_tables/double_sorts_downside_beta_ltd_equal_weighted.Rdata")

# Load the table and set the positioning

load("data/rdata/rdata_tables/double_sorts_downside_beta_ltd_equal_weighted.Rdata")
table_3 <- table_3 %>% 
  TexTools::ltx_placement(tbl_placement = "H")


TexTools::write_tex(table_3, path = "_tables/double_sorts_downside_beta_ltd_equal_weighted") 
