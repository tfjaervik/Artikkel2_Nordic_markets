...
reg_1 <- feols(lead_ret ~ ltd_nll, cluster= c("YearMonth"), data = df)
reg_2 <- feols(lead_ret ~ ltd_nll + beta, cluster = c("YearMonth"), data = df)
reg_3 <- feols(lead_ret ~ ltd_nll + beta + downside_beta, cluster = c("YearMonth"), data = df)
reg_4 <- feols(lead_ret ~ ltd_nll + beta + downside_beta + upside_beta, cluster = c("YearMonth"), data = df)
...
reg_table_1 <- etable(list(reg_1, reg_2, reg_3, reg_4),
                      ...
                      dict = c(lead_ret = "$r_{t+1}$", ltd_nll = "LTD", beta = "$\\beta$", 
...
TexTools::write_tex(reg_table_1, path = "_tables/reg_clustered_on_time_1_size_bm_nll")




reg_5 <- feols(lead_ret ~ ltd_nll + beta + downside_beta + upside_beta + coskewness,
               cluster = c("YearMonth"), data = df)
reg_6 <- feols(lead_ret ~ ltd_nll + beta + downside_beta + upside_beta + coskewness + cokurtosis,
               cluster = c("YearMonth"), data = df)


reg_7 <- feols(lead_ret ~ ltd_nll + beta + downside_beta + upside_beta + coskewness + cokurtosis + size,
               cluster = c("YearMonth"), data = df)

reg_8 <- feols(lead_ret ~ ltd_nll + beta + downside_beta + upside_beta + coskewness + cokurtosis + size + bm,
               cluster = c("YearMonth"), data = df)
...
TexTools::write_tex(reg_table_2, path = "_tables/reg_clustered_on_time_2_size_bm_nll")
